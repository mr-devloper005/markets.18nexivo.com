...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#fefddf] text-[#111]">
        <section className="border-b border-black bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#e87f24]">The newsroom</p>
            ...
            <p className="max-w-md border-l-4 border-[#e87f24] pl-5 text-sm font-bold leading-7 text-black/65">
        ...
        <section className="border-b border-black bg-[#171717] text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[#ffc81e]' : 'hover:text-[#ffc81e]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#ffc81e]' : 'whitespace-nowrap hover:text-[#ffc81e]'}>
                {item.name}
              </Link>
            ))}
          </div>
          ...
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-black lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="bg-[#e87f24] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em]">{getCategory(lead, label)}</span>
            ...
            <div className="grid">
              <div className="border-b border-black bg-[#73a5ca] p-6 text-white">
              ...
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-black bg-white last:border-b-0">
                  ...
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#e87f24]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-black bg-[#fefddf] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
            ...
            <div className="grid border-l border-t border-black md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group border-b border-r border-black bg-white">
                  ...
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.18em] text-[#e87f24]">
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            ...
            <span className="border-y border-black bg-[#e87f24] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
